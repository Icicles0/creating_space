 ...
 * @param r            radius of the orbit in 1000 km (only circles are supported)
 ...
 */
public record OrbitParameter(ResourceLocation orbitedBody,float size, float i, float omega, float r, float orbT, Vec3 rotationAxis, float rotT) {
    ...
    public static final Codec<OrbitParameter> CODEC = RecordCodecBuilder.create(
            instance ->
                    instance.group(
                            ...
                            Codec.FLOAT.optionalFieldOf("size",1.0f).forGetter(OrbitParameter::size),
